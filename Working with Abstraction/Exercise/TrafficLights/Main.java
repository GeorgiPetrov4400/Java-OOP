...
public class Main {
    public static void main(String[] args) {
        ...
        List<String> trafficLightColors = Arrays.stream(scanner.nextLine().split("\\s+")).collect(Collectors.toList());

        List<TrafficLight> trafficLights = new ArrayList<>();

        for (String color : trafficLightColors) {
        ...
        int changeTrafficLight = Integer.parseInt(scanner.nextLine());

        for (int i = 0; i < changeTrafficLight; i++) {
